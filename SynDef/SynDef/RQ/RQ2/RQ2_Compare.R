...
save.fig.dir = 'D:/cursor code/SynDef/Compare/'

dir.create(save.fig.dir, showWarnings = FALSE, recursive = TRUE)
result.dir = file.path(save.fig.dir, 'RQ2_result')
...
SynDef.result.dir = 'D:/cursor code/SynDef/SynDef_result2/'
...
{  
  ...
  # SynDef results
  SynDef.file = paste0(SynDef.result.dir, rel, '-result.csv')
  if (!file.exists(SynDef.file)) {
    warning(paste("SynDef file not found:", SynDef.file))
    SynDef.result <- NULL
  } else {
    SynDef.result = read.csv(SynDef.file)
    ...
    SynDef.result = select(SynDef.result,'filename','line.number','rank')
  }
  ...
  if (file.exists(linedef.csv_file)) {
    ...
    # Set token.attention.score to 0 for comment lines
    linedef.result[linedef.result$is.comment.line == "True",]$token.attention.score = 0

    # Get top-k tokens
    tmp.top.k.linedef <- linedef.result %>% filter(is.comment.line=="False" & file.level.ground.truth=="True" & prediction.label=="True") %>%
      group_by(test, filename) %>% top_n(1500, token.attention.score) %>% select("project","train","test","filename","token") %>% distinct()

    tmp.top.k.linedef$flag = 'topk'

    # Merge
    merged_linedef <- merge(linedef.result, tmp.top.k.linedef, by=c('project', 'train', 'test', 'filename', 'token'), all.x = TRUE)
    merged_linedef[is.na(merged_linedef$flag),]$token.attention.score = 0

    # Calculate line-level attention scores
    ...
    linedef.eval.result = get.line.metrics.result(linedef.result, cur.df.file) %>% mutate(test=rel)
  } else {
    warning(paste("LineDef file not found:", linedef.csv_file))
    linedef.eval.result = NULL
  }
  ...
  if (!is.null(SynDef.eval.result)) {
    SynDef.result.df = rbind(SynDef.result.df, SynDef.eval.result)
  }
  if (!is.null(linedef.eval.result)) {
    linedef.result.df = rbind(linedef.result.df, linedef.eval.result)
  }
...
if (!is.null(SynDef.result.df)) {
  sum_SynDef.result.df = SynDef.result.df %>% summarise(IFA=median(ifa.list),recall=median(recall.list),effort=median(effort.list), fpa=median(fpa.list), top5=mean(top5.list), top10=mean(top10.list), .by=test)
} else {
  sum_SynDef.result.df = NULL
}
if (!is.null(linedef.result.df)) {
  sum_linedef.result.df = linedef.result.df %>% summarise(IFA=median(ifa.list),recall=median(recall.list),effort=median(effort.list), fpa=median(fpa.list), top5=mean(top5.list), top10=mean(top10.list), .by=test)
} else {
  sum_linedef.result.df = NULL
}
...
if (!is.null(sum_SynDef.result.df)) {
  names(sum_SynDef.result.df) = c("release", "IFA", "Recall20%LOC", "Effort@20%Recall", "FPA", "top5", "top10")
}
if (!is.null(sum_linedef.result.df)) {
  names(sum_linedef.result.df) = c("release", "IFA", "Recall20%LOC", "Effort@20%Recall", "FPA", "top5", "top10")
}
...
write.csv(sum_splice_S.line.result, file = file.path(result.dir, 'SPLICE-S.csv'))
write.csv(sum_splice_G.line.result, file = file.path(result.dir, 'SPLICE-G.csv'))
write.csv(sum_splice_F.line.result, file = file.path(result.dir, 'SPLICE-F.csv'))
write.csv(sum_glance.lr.result.df, file = file.path(result.dir, 'GLANCE-LR.csv'))
write.csv(sum_glance.ea.result.df, file = file.path(result.dir, 'GLANCE-EA.csv'))
write.csv(sum_glance.md.result.df, file = file.path(result.dir, 'GLANCE-MD.csv'))
write.csv(sum_n.gram.result.df, file = file.path(result.dir, 'N-gram.csv'))
write.csv(sum_linedp.result.df, file = file.path(result.dir, 'LineDP.csv'))
write.csv(sum_PMD.result.df, file = file.path(result.dir, 'PMD.csv'))
write.csv(sum_deepline.dp.line.result, file = file.path(result.dir, 'DeepLineDP.csv'))
if (!is.null(sum_SynDef.result.df)) {
  write.csv(sum_SynDef.result.df, file = file.path(result.dir, 'SynDef.csv'))
}
if (!is.null(sum_linedef.result.df)) {
  write.csv(sum_linedef.result.df, file = file.path(result.dir, 'LineDef.csv'))
}
...
if (!is.null(sum_SynDef.result.df)) {
  sum_SynDef.result.df$technique = 'SynDef'
}
if (!is.null(sum_linedef.result.df)) {
  sum_linedef.result.df$technique = 'LineDef'
}
...
if (!is.null(sum_SynDef.result.df) && !is.null(sum_linedef.result.df)) {
  all.line.result = rbind(sum_glance.lr.result.df, sum_glance.ea.result.df, sum_glance.md.result.df, sum_n.gram.result.df, sum_PMD.result.df,sum_linedp.result.df,sum_deepline.dp.line.result, sum_SynDef.result.df, sum_linedef.result.df, sum_splice_S.line.result,sum_splice_G.line.result, sum_splice_F.line.result)
} else if (!is.null(sum_SynDef.result.df)) {
  all.line.result = rbind(sum_glance.lr.result.df, sum_glance.ea.result.df, sum_glance.md.result.df, sum_n.gram.result.df, sum_PMD.result.df,sum_linedp.result.df,sum_deepline.dp.line.result, sum_SynDef.result.df, sum_splice_S.line.result,sum_splice_G.line.result, sum_splice_F.line.result)
} else {
...
rq2_plot_theme = theme(
  axis.text.x = element_text(angle = -60, hjust = 0, size = 8),
  plot.margin = margin(t = 8, r = 6, b = 28, l = 10, unit = "pt")
)

ggplot(recall.result.df, aes(x=reorder(variable, -value, FUN=median), y=value)) + geom_boxplot() +
  stat_summary(fun = mean, geom = "point", shape = 17, size = 1.5, color = "red", position = position_dodge(0.75)) + facet_grid(~rank, drop=TRUE, scales = "free", space = "free") + ylab("Recall@Top20%LOC") + xlab("") + rq2_plot_theme
ggsave(paste0(save.fig.dir,"file-Recall@Top20LOC.pdf"), width = 14, height = 2.5, limitsize = FALSE)
...
ggsave(paste0(save.fig.dir,"file-Effort@Top20Recall.pdf"), width = 14, height = 2.5, limitsize = FALSE)
...
ggsave(paste0(save.fig.dir, "file-IFA.pdf"), width = 14, height = 2.5, limitsize = FALSE)
...
ggsave(paste0(save.fig.dir, "file-FPA.pdf"), width = 14, height = 2.5, limitsize = FALSE)
...
ggsave(paste0(save.fig.dir, "file-top5.pdf"), width = 14, height = 2.5, limitsize = FALSE)
...
ggsave(paste0(save.fig.dir, "file-top10.pdf"), width = 14, height = 2.5, limitsize = FALSE)
